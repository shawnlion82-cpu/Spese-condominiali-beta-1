  ...
  const handleExportPDF = () => {
    ...
    const tableData = filteredExpenses.map(e => {
        ...
        return [
            ...
            e.category,
            bankAccountName,
            new Intl.NumberFormat('it-IT', { style: 'currency', currency: 'EUR' }).format(e.amount)
        ]
    });
    const totalString = new Intl.NumberFormat('it-IT', { style: 'currency', currency: 'EUR' }).format(filteredTotal);
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden transition-colors duration-200">
        <div className="p-6 border-b border-slate-200 dark:border-slate-700 flex flex-col gap-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <h2 className="text-lg font-bold text-slate-800 dark:text-white">Storico Spese</h2>
          ...
          {showFilters && (
            <div className="pt-4 border-t border-slate-100 dark:border-slate-700 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 animate-fade-in">
              ...
              <div className="flex items-end">
                <button onClick={clearFilters} className="w-full py-2 border border-slate-200 dark:border-slate-600 text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700 rounded-lg text-sm font-medium flex items-center justify-center gap-2"><X size={16} />Resetta Filtri</button>
              </div>
            </div>
          )}
        </div>
          ...
          <table className="w-full text-left text-sm">
                ...
                filteredExpenses.map((expense) => (
                  <tr key={expense.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                    <td className="px-6 py-4 text-slate-600 dark:text-slate-300 whitespace-nowrap">
                      <div className="flex items-center gap-2">
                        <span>{new Date(expense.date).toLocaleDateString('it-IT', { day: '2-digit', month: 'short' })}</span>
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${expense.status === 'paid' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300' : 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-300'}`}>
                        ...
                        {expense.status === 'paid' ? 'Pagato' : 'Da Pagare'}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-right font-bold text-slate-800 dark:text-slate-200">
                      {new Intl.NumberFormat('it-IT', { style: 'currency', currency: 'EUR' }).format(expense.amount)}
                    </td>
                    <td className="px-6 py-4">
            ...
            {filteredExpenses.length > 0 && (
              <tfoot className="bg-slate-50 dark:bg-slate-700 border-t-2 border-slate-200 dark:border-slate-600">
                <tr>
                  <td colSpan={4} className="px-6 py-4 text-right font-bold text-slate-600 dark:text-slate-300 uppercase">Totale Filtrato</td>
                  <td className="px-6 py-4 text-right font-bold text-slate-900 dark:text-white text-base">{new Intl.NumberFormat('it-IT', { style: 'currency', currency: 'EUR' }).format(filteredTotal)}</td>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-fade-in">
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl max-w-sm w-full p-6 border border-slate-200 dark:border-slate-700">
            <div className="text-center">
              ...
              <h3 className="text-lg font-medium text-gray-900 dark:text-white mt-3">Elimina Spesa</h3>
            ...
            <div className="mt-5 sm:mt-6 sm:grid sm:grid-cols-2 sm:gap-3 sm:grid-flow-row-dense">
              ...
              <button onClick={() => setExpenseToDelete(null)} type="button" className="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 dark:border-slate-600 shadow-sm px-4 py-2 bg-white dark:bg-slate-700 text-base font-medium text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-slate-600 sm:mt-0">Annulla</button>
            </div>
          </div>
        </div>
      )}
