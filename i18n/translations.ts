

export type Language = 'it' | 'en' | 'es' | 'fr' | 'de';

export const translations = {
  it: {
    login: {
      title: 'Spese Condominiali',
      subtitle: 'La tua gestione, semplice e chiara.',
      label: 'Nome del Condominio',
      placeholder: 'ES. CONDOMINIO VIA ROMA 123',
      button: 'ACCEDI',
      footer: 'Inserisci il nome del condominio per iniziare a gestire le spese.',
      error: 'Inserisci il nome del condominio per proseguire'
    },
    nav: {
      dashboard: 'Dashboard',
      expenses: 'Lista Spese',
      incomes: 'Lista Incassi',
      accounts: 'Conti',
      reports: 'Report',
      newExpense: 'Nuova Spesa',
      newIncome: 'Nuovo Incasso',
      logout: 'Cambia Condominio',
      themeLight: 'Modalità Chiara',
      themeDark: 'Modalità Scura'
    },
    dashboard: {
      overview: 'Panoramica Esercizio',
      totalExpenses: 'Totale Spese',
      totalPaidExpenses: 'Spese Pagate',
      totalIncome: 'Totale Incassato',
      balance: 'Saldo Esercizio',
      breakdown: 'Ripartizione Spese',
      monthlyTrend: 'Andamento Spese Mensili',
      dataManagement: 'Gestione Dati ed Esportazione',
      exportBackup: 'Backup JSON',
      exportExcel: 'Esporta Excel',
      exportPDF: 'Report PDF',
      backupDesc: 'Scarica i tuoi dati nel formato che preferisci.',
      noExpenses: 'Nessuna spesa per l\'anno selezionato',
      movements: 'movimenti registrati',
      incomesRecorded: 'incassi registrati',
      diffBalance: 'Differenza tra incassi e spese',
      paidPercentage: '{percent}% del totale',
      overdueTitle: 'Attenzione: Spese Scadute',
      overdueSubtitle: 'Ci sono {count} spese non pagate con data scaduta.',
      daysOverdue: '{days} gg scaduta'
    },
    list: {
      historyExpenses: 'Storico Spese',
      historyIncomes: 'Storico Incassi',
      accounts: 'Conti Correnti',
      search: 'Cerca...',
      filters: 'Filtri avanzati',
      reset: 'Resetta',
      export: 'Esporta',
      import: 'Importa',
      importSuccess: 'Importazione completata: {count} elementi aggiunti.',
      importError: 'Errore durante l\'importazione: {error}',
      analyzing: 'Analisi del file in corso... (potrebbe richiedere qualche secondo)',
      reviewImport: 'Revisione Importazione',
      saveAll: 'Salva Tutto',
      importDesc: 'Carica un file PDF, DOCX o XML.',
      noDataImport: 'Nessun dato trovato nel file.',
      exportPDF: 'PDF',
      exportCSV: 'CSV',
      exportXML: 'XML',
      exportDOCX: 'DOCX',
      totalFiltered: 'Totale Filtrato',
      colDate: 'Data',
      colDesc: 'Descrizione',
      colCat: 'Categoria',
      colStatus: 'Stato',
      colAmount: 'Importo',
      colActions: 'Azioni',
      colAccount: 'Conto',
      colIban: 'IBAN',
      colInitBalance: 'Saldo Iniziale',
      colCurrBalance: 'Saldo Attuale',
      noData: 'Nessun dato trovato.',
      deleteAll: 'Elimina Tutto',
      confirmDeleteAllMsg: 'Sei sicuro di voler eliminare TUTTI i dati in questa lista? Questa azione è irreversibile e cancellerà definitivamente tutti i record.'
    },
    reports: {
      title: 'Report e Statistiche',
      subtitle: 'Analisi dettagliata per periodo e categoria',
      groupByMonth: 'Raggruppa per Mese',
      groupByCategory: 'Raggruppa per Categoria',
      typeExpenses: 'Spese',
      typeIncomes: 'Incassi',
      total: 'Totale',
      empty: 'Nessun dato trovato per i criteri selezionati',
      month: 'Mese',
      category: 'Categoria'
    },
    common: {
      delete: 'Elimina',
      cancel: 'Annulla',
      confirmDeleteTitle: 'Elimina Elemento',
      confirmDeleteMsg: 'Sei sicuro? L\'operazione è irreversibile.',
      paid: 'Pagato',
      unpaid: 'Da Pagare'
    }
  },
  en: {
    login: {
      title: 'Condo Expenses',
      subtitle: 'Simple and clear management.',
      label: 'Condominium Name',
      placeholder: 'EX. CONDO MAIN STREET 123',
      button: 'LOGIN',
      footer: 'Enter the condominium name to start managing expenses.',
      error: 'Enter the condominium name to continue'
    },
    nav: {
      dashboard: 'Dashboard',
      expenses: 'Expenses',
      incomes: 'Incomes',
      accounts: 'Accounts',
      reports: 'Reports',
      newExpense: 'New Expense',
      newIncome: 'New Income',
      logout: 'Switch Condo',
      themeLight: 'Light Mode',
      themeDark: 'Dark Mode'
    },
    dashboard: {
      overview: 'Overview',
      totalExpenses: 'Total Expenses',
      totalPaidExpenses: 'Paid Expenses',
      totalIncome: 'Total Income',
      balance: 'Balance',
      breakdown: 'Expenses Breakdown',
      monthlyTrend: 'Monthly Expenses Trend',
      dataManagement: 'Data Management & Export',
      exportBackup: 'JSON Backup',
      exportExcel: 'Export Excel',
      exportPDF: 'PDF Report',
      backupDesc: 'Download your data in your preferred format.',
      noExpenses: 'No expenses for the selected year',
      movements: 'recorded movements',
      incomesRecorded: 'recorded incomes',
      diffBalance: 'Difference between income and expenses',
      paidPercentage: '{percent}% of total',
      overdueTitle: 'Warning: Overdue Expenses',
      overdueSubtitle: 'There are {count} unpaid expenses past their date.',
      daysOverdue: '{days} days overdue'
    },
    list: {
      historyExpenses: 'Expenses History',
      historyIncomes: 'Incomes History',
      accounts: 'Bank Accounts',
      search: 'Search...',
      filters: 'Advanced filters',
      reset: 'Reset',
      export: 'Export',
      import: 'Import',
      importSuccess: 'Import completed: {count} items added.',
      importError: 'Import error: {error}',
      analyzing: 'Analyzing file... (this may take a few seconds)',
      reviewImport: 'Review Import',
      saveAll: 'Save All',
      importDesc: 'Upload a PDF, DOCX or XML file.',
      noDataImport: 'No data found in file.',
      exportPDF: 'PDF',
      exportCSV: 'CSV',
      exportXML: 'XML',
      exportDOCX: 'DOCX',
      totalFiltered: 'Filtered Total',
      colDate: 'Date',
      colDesc: 'Description',
      colCat: 'Category',
      colStatus: 'Status',
      colAmount: 'Amount',
      colActions: 'Actions',
      colAccount: 'Account',
      colIban: 'IBAN',
      colInitBalance: 'Init Balance',
      colCurrBalance: 'Curr Balance',
      noData: 'No data found.',
      deleteAll: 'Delete All',
      confirmDeleteAllMsg: 'Are you sure you want to delete ALL data in this list? This action is irreversible and will permanently remove all records.'
    },
    reports: {
      title: 'Reports & Stats',
      subtitle: 'Detailed analysis by period and category',
      groupByMonth: 'Group by Month',
      groupByCategory: 'Group by Category',
      typeExpenses: 'Expenses',
      typeIncomes: 'Incomes',
      total: 'Total',
      empty: 'No data found for selected criteria',
      month: 'Month',
      category: 'Category'
    },
    common: {
      delete: 'Delete',
      cancel: 'Cancel',
      confirmDeleteTitle: 'Delete Item',
      confirmDeleteMsg: 'Are you sure? This action cannot be undone.',
      paid: 'Paid',
      unpaid: 'Unpaid'
    }
  },
  es: {
    login: {
      title: 'Gastos de Condominio',
      subtitle: 'Gestión simple y clara.',
      label: 'Nombre del Condominio',
      placeholder: 'EJ. CONDOMINIO CALLE MAYOR 123',
      button: 'ACCEDER',
      footer: 'Ingresa el nombre del condominio para comenzar.',
      error: 'Ingresa el nombre del condominio para continuar'
    },
    nav: {
      dashboard: 'Tablero',
      expenses: 'Gastos',
      incomes: 'Ingresos',
      accounts: 'Cuentas',
      reports: 'Informes',
      newExpense: 'Nuevo Gasto',
      newIncome: 'Nuevo Ingreso',
      logout: 'Cambiar Condominio',
      themeLight: 'Modo Claro',
      themeDark: 'Modo Oscuro'
    },
    dashboard: {
      overview: 'Resumen del Ejercicio',
      totalExpenses: 'Total Gastos',
      totalPaidExpenses: 'Gastos Pagados',
      totalIncome: 'Total Ingresos',
      balance: 'Balance',
      breakdown: 'Desglose de Gastos',
      monthlyTrend: 'Tendencia Mensual',
      dataManagement: 'Gestión y Exportación',
      exportBackup: 'Copia JSON',
      exportExcel: 'Exportar Excel',
      exportPDF: 'Informe PDF',
      backupDesc: 'Descarga tus datos en el formato que prefieras.',
      noExpenses: 'Sin gastos para el año seleccionado',
      movements: 'movimientos registrados',
      incomesRecorded: 'ingresos registrados',
      diffBalance: 'Diferencia entre ingresos y gastos',
      paidPercentage: '{percent}% del total',
      overdueTitle: 'Atención: Gastos Vencidos',
      overdueSubtitle: 'Hay {count} gastos impagos con fecha vencida.',
      daysOverdue: '{days} días de retraso'
    },
    list: {
      historyExpenses: 'Historial de Gastos',
      historyIncomes: 'Historial de Ingresos',
      accounts: 'Cuentas Bancarias',
      search: 'Buscar...',
      filters: 'Filtros avanzados',
      reset: 'Restablecer',
      export: 'Exportar',
      import: 'Importar',
      importSuccess: 'Importación completada: {count} elementos añadidos.',
      importError: 'Error de importación: {error}',
      analyzing: 'Analizando archivo... (puede tardar unos segundos)',
      reviewImport: 'Revisar Importación',
      saveAll: 'Guardar Todo',
      importDesc: 'Sube un archivo PDF, DOCX o XML.',
      noDataImport: 'No se encontraron datos.',
      exportPDF: 'PDF',
      exportCSV: 'CSV',
      exportXML: 'XML',
      exportDOCX: 'DOCX',
      totalFiltered: 'Total Filtrado',
      colDate: 'Fecha',
      colDesc: 'Descripción',
      colCat: 'Categoría',
      colStatus: 'Estado',
      colAmount: 'Importe',
      colActions: 'Acciones',
      colAccount: 'Cuenta',
      colIban: 'IBAN',
      colInitBalance: 'Saldo Inic.',
      colCurrBalance: 'Saldo Act.',
      noData: 'No se encontraron datos.',
      deleteAll: 'Eliminar Todo',
      confirmDeleteAllMsg: '¿Estás seguro de que quieres eliminar TODOS los datos de esta lista? Esta acción es irreversible.'
    },
    reports: {
      title: 'Informes y Estadísticas',
      subtitle: 'Análisis detallado por período y categoría',
      groupByMonth: 'Agrupar por Mes',
      groupByCategory: 'Agrupar por Categoría',
      typeExpenses: 'Gastos',
      typeIncomes: 'Ingresos',
      total: 'Total',
      empty: 'No se encontraron datos',
      month: 'Mes',
      category: 'Categoría'
    },
    common: {
      delete: 'Eliminar',
      cancel: 'Cancelar',
      confirmDeleteTitle: 'Eliminar Elemento',
      confirmDeleteMsg: '¿Estás seguro? Esta acción es irreversible.',
      paid: 'Pagado',
      unpaid: 'Por Pagar'
    }
  },
  fr: {
    login: {
      title: 'Charges de Copropriété',
      subtitle: 'Gestion simple et claire.',
      label: 'Nom de la Copropriété',
      placeholder: 'EX. RÉSIDENCE ELYSÉE 123',
      button: 'ENTRER',
      footer: 'Entrez le nom de la copropriété pour commencer.',
      error: 'Entrez le nom de la copropriété pour continuer'
    },
    nav: {
      dashboard: 'Tableau de bord',
      expenses: 'Dépenses',
      incomes: 'Recettes',
      accounts: 'Comptes',
      reports: 'Rapports',
      newExpense: 'Nouvelle Dépense',
      newIncome: 'Nouvelle Recette',
      logout: 'Changer Copro',
      themeLight: 'Mode Clair',
      themeDark: 'Mode Sombre'
    },
    dashboard: {
      overview: 'Aperçu de l\'Exercice',
      totalExpenses: 'Total Dépenses',
      totalPaidExpenses: 'Dépenses Payées',
      totalIncome: 'Total Recettes',
      balance: 'Solde',
      breakdown: 'Répartition des Dépenses',
      monthlyTrend: 'Tendance Mensuelle',
      dataManagement: 'Exportation de Données',
      exportBackup: 'Sauvegarde JSON',
      exportExcel: 'Exporter Excel',
      exportPDF: 'Rapport PDF',
      backupDesc: 'Téléchargez vos données dans le format de votre choix.',
      noExpenses: 'Aucune dépense pour l\'année sélectionnée',
      movements: 'mouvements enregistrés',
      incomesRecorded: 'recettes enregistrées',
      diffBalance: 'Différence entre recettes et dépenses',
      paidPercentage: '{percent}% du total',
      overdueTitle: 'Attention: Dépenses En Retard',
      overdueSubtitle: 'Il y a {count} dépenses impayées en retard.',
      daysOverdue: '{days} j. de retard'
    },
    list: {
      historyExpenses: 'Historique Dépenses',
      historyIncomes: 'Historique Recettes',
      accounts: 'Comptes Bancaires',
      search: 'Rechercher...',
      filters: 'Filtres avancés',
      reset: 'Réinitialiser',
      export: 'Exporter',
      import: 'Importer',
      importSuccess: 'Importation terminée: {count} éléments ajoutés.',
      importError: 'Erreur d\'importation: {error}',
      analyzing: 'Analyse du fichier... (cela peut prendre quelques secondes)',
      reviewImport: 'Réviser l\'importation',
      saveAll: 'Tout Enregistrer',
      importDesc: 'Téléchargez un fichier PDF, DOCX ou XML.',
      noDataImport: 'Aucune donnée trouvée.',
      exportPDF: 'PDF',
      exportCSV: 'CSV',
      exportXML: 'XML',
      exportDOCX: 'DOCX',
      totalFiltered: 'Total Filtré',
      colDate: 'Date',
      colDesc: 'Description',
      colCat: 'Catégorie',
      colStatus: 'Statut',
      colAmount: 'Montant',
      colActions: 'Actions',
      colAccount: 'Compte',
      colIban: 'IBAN',
      colInitBalance: 'Solde Init.',
      colCurrBalance: 'Solde Act.',
      noData: 'Aucune donnée trouvée.',
      deleteAll: 'Tout Supprimer',
      confirmDeleteAllMsg: 'Êtes-vous sûr de vouloir supprimer TOUTES les données de cette liste ? Cette action est irréversible.'
    },
    reports: {
      title: 'Rapports et Statistiques',
      subtitle: 'Analyse détaillée par période et catégorie',
      groupByMonth: 'Grouper par Mois',
      groupByCategory: 'Grouper par Catégorie',
      typeExpenses: 'Dépenses',
      typeIncomes: 'Recettes',
      total: 'Total',
      empty: 'Aucune donnée trouvée',
      month: 'Mois',
      category: 'Catégorie'
    },
    common: {
      delete: 'Supprimer',
      cancel: 'Annuler',
      confirmDeleteTitle: 'Supprimer Élément',
      confirmDeleteMsg: 'Êtes-vous sûr ? Cette action est irréversible.',
      paid: 'Payé',
      unpaid: 'À Payer'
    }
  },
  de: {
    login: {
      title: 'Hausverwaltung',
      subtitle: 'Einfache und klare Verwaltung.',
      label: 'Name der Wohnanlage',
      placeholder: 'B.SP. RESIDENZ MÜNCHEN 123',
      button: 'ANMELDEN',
      footer: 'Geben Sie den Namen der Wohnanlage ein, um zu beginnen.',
      error: 'Geben Sie den Namen der Wohnanlage ein, um fortzufahren'
    },
    nav: {
      dashboard: 'Dashboard',
      expenses: 'Ausgaben',
      incomes: 'Einnahmen',
      accounts: 'Konten',
      reports: 'Berichte',
      newExpense: 'Neue Ausgabe',
      newIncome: 'Neue Einnahme',
      logout: 'Wechseln',
      themeLight: 'Heller Modus',
      themeDark: 'Dunkler Modus'
    },
    dashboard: {
      overview: 'Übersicht',
      totalExpenses: 'Gesamtausgaben',
      totalPaidExpenses: 'Bezahlte Ausgaben',
      totalIncome: 'Gesamteinnahmen',
      balance: 'Saldo',
      breakdown: 'Ausgabenaufschlüsselung',
      monthlyTrend: 'Monatlicher Trend',
      dataManagement: 'Datenexport',
      exportBackup: 'JSON Backup',
      exportExcel: 'Excel Export',
      exportPDF: 'PDF Bericht',
      backupDesc: 'Laden Sie Ihre Daten im gewünschten Format herunter.',
      noExpenses: 'Keine Ausgaben für das ausgewählte Jahr',
      movements: 'registrierte Bewegungen',
      incomesRecorded: 'registrierte Einnahmen',
      diffBalance: 'Differenz zwischen Einnahmen und Ausgaben',
      paidPercentage: '{percent}% der Gesamtmenge',
      overdueTitle: 'Achtung: Überfällige Ausgaben',
      overdueSubtitle: 'Es gibt {count} unbezahlte Ausgaben, deren Datum überschritten ist.',
      daysOverdue: '{days} Tage überfällig'
    },
    list: {
      historyExpenses: 'Ausgabenverlauf',
      historyIncomes: 'Einnahmenverlauf',
      accounts: 'Bankkonten',
      search: 'Suchen...',
      filters: 'Erweiterte Filter',
      reset: 'Zurücksetzen',
      export: 'Export',
      import: 'Importieren',
      importSuccess: 'Import abgeschlossen: {count} Elemente hinzugefügt.',
      importError: 'Importfehler: {error}',
      analyzing: 'Datei wird analysiert... (dies kann einige Sekunden dauern)',
      reviewImport: 'Import überprüfen',
      saveAll: 'Alle speichern',
      importDesc: 'Laden Sie eine PDF, DOCX oder XML Datei hoch.',
      noDataImport: 'Keine Daten gefunden.',
      exportPDF: 'PDF',
      exportCSV: 'CSV',
      exportXML: 'XML',
      exportDOCX: 'DOCX',
      totalFiltered: 'Gefilterte Summe',
      colDate: 'Datum',
      colDesc: 'Beschreibung',
      colCat: 'Kategorie',
      colStatus: 'Status',
      colAmount: 'Betrag',
      colActions: 'Aktionen',
      colAccount: 'Konto',
      colIban: 'IBAN',
      colInitBalance: 'Anfangssaldo',
      colCurrBalance: 'Akt. Saldo',
      noData: 'Keine Daten gefunden.',
      deleteAll: 'Alle Löschen',
      confirmDeleteAllMsg: 'Sind Sie sicher, dass Sie ALLE Daten in dieser Liste löschen möchten? Diese Aktion ist irreversibel.'
    },
    reports: {
      title: 'Berichte und Statistiken',
      subtitle: 'Detaillierte Analyse nach Zeitraum und Kategorie',
      groupByMonth: 'Nach Monat gruppieren',
      groupByCategory: 'Nach Kategorie gruppieren',
      typeExpenses: 'Ausgaben',
      typeIncomes: 'Einnahmen',
      total: 'Gesamt',
      empty: 'Keine Daten gefunden',
      month: 'Monat',
      category: 'Kategorie'
    },
    common: {
      delete: 'Löschen',
      cancel: 'Abbrechen',
      confirmDeleteTitle: 'Element löschen',
      confirmDeleteMsg: 'Sind Sie sicher? Diese Aktion kann nicht rückgängig gemacht werden.',
      paid: 'Bezahlt',
      unpaid: 'Unbezahlt'
    }
  }
};
